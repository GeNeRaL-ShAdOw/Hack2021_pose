...
class Pose:
    ...
    def draw(self, img):
        ...
        for part_id in range(0,17): #len(BODY_PARTS_PAF_IDS) - 2
            ...
            if global_kpt_a_id != -1:
                ...
                cv2.circle(black, (int(x_a), int(y_a)), 3, Pose.color, -1)

                cv2.circle(canny, (int(x_a), int(y_a)), 3, Pose.color, -1)
            ...
            if global_kpt_b_id != -1:
                ...
                cv2.circle(black, (int(x_b), int(y_b)), 3, Pose.color, -1)

                cv2.circle(canny, (int(x_b), int(y_b)), 3, Pose.color, -1)

            if global_kpt_a_id != -1 and global_kpt_b_id != -1:
                cv2.line(img, (int(x_a), int(y_a)), (int(x_b), int(y_b)), (255,0,0), 2)

                cv2.line(black, (int(x_a), int(y_a)), (int(x_b), int(y_b)), (255,0,0), 2)

                cv2.line(canny, (int(x_a), int(y_a)), (int(x_b), int(y_b)), (255,0,0), 2)

                dist = int(((x_a - x_b)**2 + (y_a - y_b)**2)**0.5)
                cv2.putText(img, str(dist), (int((x_a+x_b)/2), int((y_a+y_b)/2)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255), 2, cv2.LINE_AA)

                cv2.putText(black, str(dist), (int((x_a+x_b)/2), int((y_a+y_b)/2)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255), 2, cv2.LINE_AA)
